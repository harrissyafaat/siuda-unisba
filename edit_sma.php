    ...
    <div class="content">
      <div class="content_bg">
        <div class="mainbar">
		  ...
		  <div>
		  <?php
		  $edit = mysql_query("SELECT * FROM tb_psb_sma WHERE nisn='$_GET[nisn]'");
		  $r    = mysql_fetch_array($edit);
		  $sma = mysql_query('SELECT * FROM tb_sma order by id_sma');
		  $option = '';
		  while($a=mysql_fetch_array($sma)){
		  if($a['id_sma']==$r['id_sma']){
			$p="selected";
		  }
		  else{
			$p="";
		  }
		  $option .= "<option $p value='".$a["id_sma"]."'>".$a['nama_sma']."</option>";
		  }
		  echo "<h2>Edit Siswa SMA</h2>
		  <form method='post' action='aksi_update_sma.php'>
          <table>
		  ...
		  <tr><td>SMA </td><td> : <select name='id_sma'>$option</select><input type=hidden name=id_sma_lama value='$r[id_sma]'>
